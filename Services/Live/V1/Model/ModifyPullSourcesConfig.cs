...
namespace HuaweiCloud.SDK.Live.V1.Model
{
    ...
    public class ModifyPullSourcesConfig 
    {
        ...
        /// </summary>
        /// <value>回源方式。  包含如下取值： - domain: 回源客户源站，源站地址是域名格式。回源域名，可配置多个，如果回源失败，将按照配置顺序进行轮循。 - ipaddr: 回源客户源站，源站地址是IP格式。回源IP，可配置多个，如果回源失败，将按照配置顺序进行轮循。同时，最多可以配置一个回源域名，如果配置，回源时httpflv HOST头填该域名，RTMP tcurl字段填该域名，否则按当前IP作为HOST。 - huawei: 回源华为源站，域名创建后的默认值。</value>
        ...
        /// </summary>
        /// <value>回源协议，回源方式非“huawei”时必选。  包含如下取值： - http - rtmp</value>
        ...
        {
            return this.Equals(input as ModifyPullSourcesConfig);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        public bool Equals(ModifyPullSourcesConfig input)
        {
            if (input == null)
                return false;
