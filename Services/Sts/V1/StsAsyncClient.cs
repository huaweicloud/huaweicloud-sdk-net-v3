...
namespace HuaweiCloud.SDK.Sts.V1
{
    public partial class StsAsyncClient : Client
    {
        ...
        /// Please refer to HUAWEI cloud API Explorer for details.
        /// </summary>
        public async Task<AssumeAgencyWithOIDCResponse> AssumeAgencyWithOIDCAsync(AssumeAgencyWithOIDCRequest assumeAgencyWithOIDCRequest)
        {
            var urlParam = new Dictionary<string, string>();
            var urlPath = HttpUtils.AddUrlPath("/v5/agencies/assume-with-oidc", urlParam);
            var request = HttpUtils.InitSdkRequest(urlPath, "application/json", assumeAgencyWithOIDCRequest);
            var response = await DoHttpRequestAsync("POST", request);
            return JsonUtils.DeSerialize<AssumeAgencyWithOIDCResponse>(response);
        }

        public AsyncInvoker<AssumeAgencyWithOIDCResponse> AssumeAgencyWithOIDCAsyncInvoker(AssumeAgencyWithOIDCRequest assumeAgencyWithOIDCRequest)
        {
            var urlParam = new Dictionary<string, string>();
            var urlPath = HttpUtils.AddUrlPath("/v5/agencies/assume-with-oidc", urlParam);
            var request = HttpUtils.InitSdkRequest(urlPath, "application/json", assumeAgencyWithOIDCRequest);
            return new AsyncInvoker<AssumeAgencyWithOIDCResponse>(this, "POST", request, JsonUtils.DeSerialize<AssumeAgencyWithOIDCResponse>);
        }
        ...
        /// Please refer to HUAWEI cloud API Explorer for details.
        /// </summary>
        public async Task<AssumeAgencyWithSAMLResponse> AssumeAgencyWithSAMLAsync(AssumeAgencyWithSAMLRequest assumeAgencyWithSAMLRequest)
        {
            var urlParam = new Dictionary<string, string>();
            var urlPath = HttpUtils.AddUrlPath("/v5/agencies/assume-with-saml", urlParam);
            var request = HttpUtils.InitSdkRequest(urlPath, "application/json", assumeAgencyWithSAMLRequest);
            var response = await DoHttpRequestAsync("POST", request);
            return JsonUtils.DeSerialize<AssumeAgencyWithSAMLResponse>(response);
        }

        public AsyncInvoker<AssumeAgencyWithSAMLResponse> AssumeAgencyWithSAMLAsyncInvoker(AssumeAgencyWithSAMLRequest assumeAgencyWithSAMLRequest)
        {
            var urlParam = new Dictionary<string, string>();
            var urlPath = HttpUtils.AddUrlPath("/v5/agencies/assume-with-saml", urlParam);
            var request = HttpUtils.InitSdkRequest(urlPath, "application/json", assumeAgencyWithSAMLRequest);
            return new AsyncInvoker<AssumeAgencyWithSAMLResponse>(this, "POST", request, JsonUtils.DeSerialize<AssumeAgencyWithSAMLResponse>);
        }
