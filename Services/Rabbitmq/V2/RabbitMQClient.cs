...
namespace HuaweiCloud.SDK.Rabbitmq.V2
{
    public partial class RabbitmqClient : Client
    {
        ...
        public SyncInvoker<ShowRabbitMqProductCoresResponse> ShowRabbitMqProductCoresInvoker(ShowRabbitMqProductCoresRequest showRabbitMqProductCoresRequest)
        {
            var urlParam = new Dictionary<string, string>();
            var urlPath = HttpUtils.AddUrlPath("/v2/rabbitmq/products/cores", urlParam);
            var request = HttpUtils.InitSdkRequest(urlPath, "application/json", showRabbitMqProductCoresRequest);
            return new SyncInvoker<ShowRabbitMqProductCoresResponse>(this, "GET", request, JsonUtils.DeSerialize<ShowRabbitMqProductCoresResponse>);
        }
