...
namespace HuaweiCloud.SDK.Rabbitmq.V2
{
    public class RabbitMQRegion
    {
        ...
        public static Region ValueOf(string regionId)
        {
            ...
            throw new ArgumentException($"region id '{regionId}' is not in the following supported regions of service 'RabbitMQ': [{string.Join(", ", StaticFields.Keys.OrderBy(key => key))}]");
        }
    }
}