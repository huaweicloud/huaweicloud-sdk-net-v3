...
namespace HuaweiCloud.SDK.Elb.V3.Model
{
    ...
    public class CreateLoadBalancerBandwidthOption 
    {
        ...
        /// </summary>
        /// <value>**参数解释**：计费模式。  **约束限制**：当id字段为null时，charge_mode是必须的。 [当前仅支持traffic按流量计费。](tag:hws_eu,g42,hk_g42,dt,hcso_dt)  **取值范围**：  - bandwidth：按带宽计费。  - traffic： 按流量计费。  **默认取值**：不涉及</value>
        ...
        /// </summary>
        /// <value>**参数解释**：带宽类型。  **约束限制**： - 当id字段为null时，share_type是必须的。当id不为null时，该字段被忽略。 - 该字段为WHOLE时,必须指定带宽ID。 - IPv6的EIP不支持WHOLE类型带宽。  **取值范围**： - PER：独享带宽。 - WHOLE：共享带宽。  **默认取值**：不涉及</value>
