...
namespace HuaweiCloud.SDK.Elb.V3.Model
{
    ...
    public class CreateLoadBalancerOption 
    {
        ...
        /// </summary>
        /// <value>参数解释：负载均衡器实例的计费模式。flavor: 固定规格计费，默认值。lcu: 弹性规格计费（按用户真实使用的lcu个数计费）。  约束限制：不建议用户传该字段。系统会基于用户传入的l4_flavor_id/l7_flavor_id的规格类型，自动识别计费模式。  取值范围：    - flavor: 固定规格计费。    - lcu: 弹性规格计费（按用户真实使用的lcu个数计费）。  默认取值：flavor。</value>
