using System;
using System.Collections.Generic;
using System.Text;
using System.Linq;
using System.Runtime.Serialization;

using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using HuaweiCloud.SDK.Core;

namespace HuaweiCloud.SDK.Rds.V3.Model
{
    /// <summary>
    /// Response Object
    /// </summary>
    public class ShowHotClodSeparationStatusResponse : SdkResponse
    {

        /// <summary>
        /// 功能是否开启
        /// </summary>
        [JsonProperty("opened", NullValueHandling = NullValueHandling.Ignore)]
        public bool? Opened { get; set; }



        /// <summary>
        /// Get the string
        /// </summary>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class ShowHotClodSeparationStatusResponse {\n");
            sb.Append("  opened: ").Append(Opened).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        public override bool Equals(object input)
        {
            return this.Equals(input as ShowHotClodSeparationStatusResponse);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        public bool Equals(ShowHotClodSeparationStatusResponse input)
        {
            if (input == null) return false;
            if (this.Opened != input.Opened || (this.Opened != null && !this.Opened.Equals(input.Opened))) return false;

            return true;
        }

        /// <summary>
        /// Get hash code
        /// </summary>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                var hashCode = 41;
                if (this.Opened != null) hashCode = hashCode * 59 + this.Opened.GetHashCode();
                return hashCode;
            }
        }
    }
}
